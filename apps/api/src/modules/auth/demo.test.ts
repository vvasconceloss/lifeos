...
import { DEMO_EMAIL } from './auth.service';
import { describe, expect, it, beforeAll, afterAll } from 'vitest';

const DEMO_TIMEOUT = 20000;
...
describe('POST /v1/auth/demo', () => {
  ...
  it(
    'logs into a seeded demo account',
    async () => {
  ...
  it(
    'seeds sample data on the demo account',
    async () => {
      const app = await buildApp({ csrf: false });
  ...
  it(
    'returns a consistent account across calls',
    async () => {
      const app = await buildApp({ csrf: false });
  ...
  it(
    'requires no body and is unauthenticated-friendly',
    async () => {
      const app = await buildApp({ csrf: false });
